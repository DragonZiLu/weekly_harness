"""
ZZ800 万宝路挖掘 — 10年股息再投评估
===========================================
对象：中证800成分股（2015年前上市）
窗口：2015-01-05 → 2025-01-05（10年）
"""
...
print("=" * 70)
print("  ZZ800 万宝路挖掘 — 10年股息再投扫描 (2015-2025)")
print("=" * 70)
...
print(f"  2015年前上市: {len(codes_filtered)} 只 (剔除 {len(codes) - len(codes_filtered)} 只)")
...
if results:
    ...
    print(f"  💾 全部 {len(results)} 只结果 → data/marlboro_zz800_10y_all.csv")
...
if stable:
    print(f"\n  {'─' * 100}")
    print(f"  📊 稳健红利型 (10年窗口) — {len(stable)} 只")
    print(f"  {'─' * 100}")
    for r in stable[:15]:
        div_pct = r['div_contrib'] / r['total_ret'] * 100 if r['total_ret'] > 0 else 0
        print(f"  {r['name']:<8} {r['code']:<12} {r['total_ret']:>+6.1f}% {r['cagr']:>+5.1f}% "
...
if not marlboro_df.empty:
    marlboro_df.to_csv(output_dir / "marlboro_zz800_10y_pure.csv", index=False, encoding='utf-8-sig')
    print(f"\n  💾 万宝路型 {len(marlboro)} 只 → data/marlboro_zz800_10y_pure.csv")

stable_df = pd.DataFrame(stable)
if not stable_df.empty:
    stable_df.to_csv(output_dir / "marlboro_zz800_10y_stable.csv", index=False, encoding='utf-8-sig')
    print(f"  💾 稳健型 {len(stable)} 只 → data/marlboro_zz800_10y_stable.csv")
