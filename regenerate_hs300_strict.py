#!/usr/bin/env python3
"""Regenerate HS300 FCF baskets with strict 5yr OCF rule"""
...
# Use ZZ800 dates as reference (they share semi-annual schedule)
old_path = Path(__file__).resolve().parent / "output" / "zz800_fcf_fixed" / "all_baskets_2015_2026.json"
...
for i, date in enumerate(dates):
    ...
    codes = [s["ts_code"] for s in clean] if n > 0 else []
    cnooc = "600938.SH" in codes
    cmcc = "600941.SH" in codes
    flag = ""
    if cnooc: flag += " ⚠️海油"
    if cmcc: flag += " ⚠️移动"
    if not flag and n > 0: flag = " ✓"
