"""
ZZ800 万宝路挖掘 — 5年股息再投评估
===========================================
对象：中证800成分股（2021年前上市）
窗口：2021-01-04 → 2026-01-04（5年）
"""
...
BUY_DATE = "2021-01-04"
YEARS = 5
LIST_CUTOFF = "20210101"
INDEX_CODE = "000906.SH"  # ZZ800

DIV_CONTRIB_THRESHOLD = 0.5
DIV_RATIO_THRESHOLD = 25
MIN_TOTAL_RET = 0

print("=" * 70)
print("  ZZ800 万宝路挖掘 — 5年股息再投扫描 (2021-2026)")
print("=" * 70)
...
try:
    ...
    df = pro.index_weight(index_code=INDEX_CODE, trade_date='20260508')
    codes = sorted(set(df['con_code'].tolist()))
...
if results:
    ...
    df_all.to_csv(output_dir / "marlboro_zz800_5y_all.csv", index=False, encoding='utf-8-sig')
    print(f"  💾 全部 {len(results)} 只结果 → data/marlboro_zz800_5y_all.csv")
...
if stable:
    print(f"\n  {'─' * 100}")
    print(f"  📊 稳健红利型 (5年窗口) — {len(stable)} 只")
    print(f"  {'─' * 100}")
    for r in stable[:15]:
        div_pct = r['div_contrib'] / r['total_ret'] * 100 if r['total_ret'] > 0 else 0
        print(f"  {r['name']:<8} {r['code']:<12} {r['total_ret']:>+6.1f}% {r['cagr']:>+5.1f}% "
...
print(f"\n\n  {'─' * 100}")
print(f"  🏆 ZZ800 股息再投 5年总收益 Top 20")
print(f"  {'─' * 100}")
...
if not marlboro_df.empty:
    marlboro_df.to_csv(output_dir / "marlboro_zz800_5y_pure.csv", index=False, encoding='utf-8-sig')
    print(f"\n  💾 万宝路型 {len(marlboro)} 只 → data/marlboro_zz800_5y_pure.csv")

stable_df = pd.DataFrame(stable)
if not stable_df.empty:
    stable_df.to_csv(output_dir / "marlboro_zz800_5y_stable.csv", index=False, encoding='utf-8-sig')
    print(f"  💾 稳健型 {len(stable)} 只 → data/marlboro_zz800_5y_stable.csv")
