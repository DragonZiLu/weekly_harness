#!/usr/bin/env python3
"""generate_f50_baskets.py — F版篮子生成（缓冲区±50%）"""
...
TOP_N        = 50
BUFFER_RATIO = 0.50
LOW_BOUND    = int(TOP_N * (1 - BUFFER_RATIO))   # 25
HIGH_BOUND   = int(TOP_N * (1 + BUFFER_RATIO))   # 75

OUT_DIR = PROJECT_ROOT / "output" / "zz800_fcf_lenient_buffer_f50"
...
print(f"F版(±50%缓冲区): TOP_N={TOP_N}, LOW={LOW_BOUND}, HIGH={HIGH_BOUND}")
...
for i, date_str in enumerate(REBALANCE_DATES):
    try:
        ...
        if i == 0 or not prev_codes:
            ...
            stocks = [dict(s) for s in apply_buffer(ranked, prev_codes, LOW_BOUND, HIGH_BOUND, TOP_N)]

        fcf_weights(stocks)
        f_baskets[date_str] = stocks
...
with open(OUT_DIR / "all_baskets_2015_2026.json", "w") as f:
    json.dump(f_baskets, f, ensure_ascii=False, indent=2)

valid = sum(1 for d in f_baskets if len(f_baskets[d]) >= 10)
print(f"\n✅ F版篮子已保存: {OUT_DIR}/all_baskets_2015_2026.json")
print(f"   有效期数: {valid}/{len(f_baskets)}  总耗时: {time.time()-t0:.0f}s")
